...
/**
 * BusinessCreateForm
 * - supports create, edit (prefill), copy (prefill but treated as create)
 * - reads edit/copy id from query params and consumes context (useBusinessContext)
 *
 * Notes:
 * - Category mapping: getBusinessCategories() returns items with .value = KEY and .label = display name.
 *   Businesses from API might contain either the KEY or a human-readable category string. We attempt
 *   to resolve to the KEY so the Select can pre-select correctly.
 */
export default function BusinessCreateForm() {
    const router = useRouter()
    const searchParams = useSearchParams()
    const { user } = useAuth()
    const { editBusiness, copyBusiness, clearEditBusiness, clearCopyBusiness } = useBusinessContext()
    ...
    const isCopyMode = !!copyId && !isEditMode
    ...
    // pendingCategoryCandidate used when prefilling before categories loaded
    ...
    // When categories arrive, if we have a pending candidate from prefill, try to resolve it to a key
    useEffect(() => {
        if (!pendingCategoryCandidateRef.current) return
        if (!categories || categories.length === 0) return

        const candidate = pendingCategoryCandidateRef.current
        const resolved = resolveCategoryKey(candidate, categories)
        if (resolved) {
            setFormData(prev => ({ ...prev, category: resolved }))
        }
        pendingCategoryCandidateRef.current = null
    }, [categories])

    // Prefill from context when editing or copying
    useEffect(() => {
        if (isEditMode && editBusiness && String(editBusiness.id) === String(editId)) {
            prefillFromBusinessObject(editBusiness, { keepId: true })
            return
        }

        if (isCopyMode && copyBusiness && String(copyBusiness.id) === String(copyId)) {
            prefillFromBusinessObject(copyBusiness, { keepId: false })
            return
        }

        // if edit/copy param present but no context, we do not fetch here.
    }, [isEditMode, isCopyMode, editId, copyId, editBusiness, copyBusiness])
    ...
    const resolveCategoryKey = (bizCategoryValueOrKey, categoriesList) => {
        ...
        // 1) If category is already a key that matches value
        const byValue = categoriesList.find(c => String(c.value) === normalized)
        if (byValue) return byValue.value

        // 2) Try match label (display name) -> contains or equal (case-insensitive)
        ...
        // 3) If biz sent label like "Restaurant/Nhà Hàng" which may contain the label start, try contains
        const partial = categoriesList.find(c => (c.label || '').toLowerCase().indexOf(lower) !== -1 || lower.indexOf((c.label || '').toLowerCase()) !== -1)
        if (partial) return partial.value

        // 4) fallback: try extracting uppercase words without spaces as key (best-effort)
    ...
    const prefillFromBusinessObject = (biz, { keepId = false } = {}) => {
        try {
            // If categories available, resolve to key, otherwise keep candidate for later
            let categoryKey = ''
            ...
            const newForm = {
                ...
                category: categoryKey || (biz.category || ''), // if unresolved, put raw (will be resolved later if possible)
            ...
            // build socialLinks array for UI if social links exist as object
            const linksArr = []
            ...
            // location input used by LocationAutoComplete
            setLocationInput(newForm.address_info.fullAddress || newForm.address_info.address || '')

            // if keepId === false (copy) we should clear slug so user chooses new slug
            if (!keepId) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // NOTE: using create endpoint for now. If update endpoint exists, branch by isEditMode.
            const result = await businessHandlers.createBusinessByUser(payload)

            if (result.success) {
                alert(isEditMode ? 'Business updated successfully!' : 'Business created successfully!')

                // clear context edit/copy after submit if present
                if (isEditMode) {
                    try { clearEditBusiness?.() } catch (e) { /* ignore */ }
                }
                if (isCopyMode) {
                    try { clearCopyBusiness?.() } catch (e) { /* ignore */ }
                }

                router.push('/business')
            } else {
                throw new Error(result.message || 'Failed to save business')
            }
    ...
    // Button label: Update if ?edit, else Create (including copy)
    const submitLabel = isEditMode ? 'Update Business' : 'Create Business'
                ...
                <div className="mx-auto max-w-4xl px-6 py-8">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div className="flex items-center justify-end gap-4 border-t border-gray-200 pt-4">
                            ...
                            >
                                {isSubmitting ? (isEditMode ? 'Updating...' : 'Creating...') : submitLabel}
                            </Button>
                        </div>
                    </form>
                </div>
